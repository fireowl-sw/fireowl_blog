...
function generateToc(content: string): { id: string; text: string; level: number }[] {
  ...
  // 先移除代码块，避免代码块中的 # 被误识别为标题
  const contentWithoutCode = content.replace(/```[\s\S]*?```/g, '').replace(/~~~[\s\S]*?~~~/g, '')
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex gap-8">
          ...
          {toc.length > 0 && (
            <aside
              className={`hidden lg:block flex-shrink-0 transition-all duration-300 ease-in-out ${
                tocCollapsed ? 'w-0 opacity-0' : 'w-64 opacity-100'
              }`}
            >
              <nav className="sticky top-32 px-2 overflow-y-auto max-h-[calc(100vh-10rem)]">
                {/* 目录标题 + 折叠按钮 */}
          ...
          <div className="flex-1">
            <div className="max-w-4xl transition-all duration-300 ease-in-out" style={{
              marginLeft: tocCollapsed ? '10rem' : '0',
              marginRight: tocCollapsed ? '10rem' : '0'
            }}>
              {/* 文章头部 */}
        ...
        {toc.length > 0 && tocCollapsed && (
          <button
            onClick={() => setTocCollapsed(!tocCollapsed)}
            className="hidden lg:flex fixed left-0 top-1/2 -translate-y-1/2 bg-card border border-gray-700 border-l-0 rounded-r-lg p-2 text-gray-400 hover:text-gray-200 hover:bg-gray-800 transition-all z-10"
            aria-label="展开目录"
          >
